...
| `run_tagging_qa.py` | _(none — read-only; prints + optional JSON)_ | `--timeframe`, `--start`, `--end`, `--min-volume` (def 20), `--top` (def 15), `--sample-limit` (def 25), `--json-output` | **Depends on `cxone_transcript_analysis` + `combined_interactions` + `analytics_reason_taxonomy`** (via analytics views). Tagging-accuracy QA: compares the agent-tagged Zendesk reason against the transcript-derived reason (both canonicalized) to flag miscategorized tickets. No LLM/creds needed. Reads `analytics_interaction_outcomes` / `analytics_reason_mismatches`; run `build_reason_taxonomy` first for meaningful canonical labels. |
| `build_reason_taxonomy.py` | `analytics_reason_taxonomy` (canonical reason map); refreshes analytics views | `--config` (def `config/reason_taxonomy.json`), `--database-url`/`--target-url`, `--no-prune`, `--refresh-views/--no-refresh-views` | **Depends on `cxone_transcript_analysis` and/or `combined_interactions`.** No LLM needed. Resolves free-text reasons to canonical labels so the `*_canonical` columns / `analytics_canonical_reason_outcomes` / `analytics_reason_reconciliation` views work. Re-run after editing `config/reason_taxonomy.json`. `run_daily_pipeline` runs it automatically. |
...
| `run_daily_pipeline.py` | Orchestrates: `cxone_transcripts`, `zendesk_tickets`, `combined_interactions`, `cxone_transcript_analysis`, `transcript_reduction_reports`/`_reasons`, `analytics_reason_taxonomy`, `analytics_knowledge_chunks` | `--date`, `--timezone` (def UTC), `--zendesk-lookback-days` (def 2), `--skip-cxone`, `--skip-zendesk`, `--skip-combined`, `--skip-classification`, `--skip-reason-taxonomy`, `--skip-knowledge-index`, `--dry-run`, `--sync-railway` | **One command that runs steps 1–5 in the correct order.** Needs `init_db` once + CXone/Zendesk creds + `OPENAI_API_KEY` (for classification + index). `--sync-railway` needs `TARGET_DATABASE_URL` (also rebuilds the reason taxonomy on Railway). |
